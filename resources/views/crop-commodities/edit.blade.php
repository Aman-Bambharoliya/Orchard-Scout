            ...
            <div class="card mb-5 mb-xl-10">
                <div id="kt_account_settings_profile_details" class="collapse show">
                    <form id="add_frm" name='add_frm' class="form" method="post"
                        action="{{ route('crop-commodities.update', $data->id) }}">
                        @csrf
                        @method('PUT')
                                        ...
                                        @if (!empty($CropCommodityTypes) && count($CropCommodityTypes) > 0)
                                            @foreach ($CropCommodityTypes as $CropCommodityType)
                                                <option value='{{ $CropCommodityType->id }}'
                                                    @if ($data->crop_commodity_type_id == $CropCommodityType->id) {{ 'selected' }} @endif>
                                    ...
                                    @if ($question->getItemOptionAttributes->isNotEmpty())
                                        ...
                                        <div class="py-3" data-kt-customer-payment-method="row">
                                            ...
                                            <div id="kt_customer_view_payment_method_{{ $question->id }}"
                                                class="fs-6 ps-10 collapse {{ $question->scout_report_category_name }} @php $questionAttributeIdArray = $question->getItemOptionAttributes->pluck('id')->toArray();
                                                    $dataCropCommodityItemAttributeIdArray = $data->hasCropCommodityQuestionItemAttribute->pluck('question_item_attribute_id')->toArray();
    
                                                    if(count(array_intersect($questionAttributeIdArray,$dataCropCommodityItemAttributeIdArray)) > 0){
                                                        echo "show";
                                                    } @endphp"
                                                style="">
                                                            ...
                                                            <div data-repeater-item="" class="option-item-wrap">
                                                                @if (count($question->getItemOptionAttributes) > 1)
                                                                    <div class="question_item_wrapper ">
                                                                        <div class="checkbox-wrapper form-group row mb-5  ">
                                                                            <div class="col-md-12">
                                                                                <div class="form-check">
                                                                                    <input
                                                                                        ...
                                                                                        @php $questionAttributeIdArray = $question->getItemOptionAttributes->pluck('id')->toArray();
                                                                                        $dataCropCommodityItemAttributeIdArray = $data->hasCropCommodityQuestionItemAttribute->pluck('question_item_attribute_id')->toArray();
                                                                                            if(count($questionAttributeIdArray) == count(array_intersect($dataCropCommodityItemAttributeIdArray,$questionAttributeIdArray))){
                                                                ...
                                                                @foreach ($question->getItemOptionAttributes as $item)
                                                                    <div class="question_item_wrapper child ">
                                                                        <div class="checkbox-wrapper form-group row mb-5  ">
                                                                            <div class="col-md-12">
                                                                                <div class="form-check">
                                                                                    <input
                                                                                        ...
                                                                                        @php $ids = $data->hasCropCommodityQuestionItemAttribute;
                                                                                        foreach($ids as $id){
                                                                                            if($id->question_item_attribute_id == $item->id){
                                                                                            echo 'checked';
                                                                                            }
                                                                                        } @endphp>
                                                                                    <label class="form-check-label"
